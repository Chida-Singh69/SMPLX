...
Examples:
  1) Build latent cache from datasett:
     python vae_inference.py cache --pkl-dir datasett --ckpt checkpoints/vae/vae_best.pt --stats checkpoints/vae/norm_stats.npz
...
from pose_dataset import load_pickle_cpu, load_stats
...
def parse_args():
    ...
    cache_cmd.add_argument("--pkl-dir", type=str, default="datasett")
    cache_cmd.add_argument("--ckpt", type=str, required=True)
    cache_cmd.add_argument("--stats", type=str, required=True)
    cache_cmd.add_argument("--out", type=str, default="checkpoints/vae/latent_cache.npz")
